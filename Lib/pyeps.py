''' pylib is a python function library for 
    creating numpy arrays using either normal cpu memeory
    or unified gpu memeory and converting between 
    numpy and eps arrays.
    It is intended to be used as support for the automatic
    python/eps wrapper functions generated by the eps compiler.
    ...
    Functions: 
          ...
          The main features of the functions listed above are:
            ...
            2. The library works on both cpu and gpu using cpu memory 
               and unified memeory.
          
          The only requirement is that numpy arrays must be created with the
          Izeros and Fzeros functions. A numpy array created with f.ex np.zeros
          will not work.  The main reason for this is the distinction between
          cpu and gpu memory.  Data which is used by the gpu must be allocated
          using unified memory, which is done by Izeros and Fzeros.  It would
          be entirely possible to use normal numpy arrays created with f.ex.
          np.zeros, but then a full copy would have to be made of all functions
          ...
          I have used different names for the num and eps functions,
          although they are identical. I will keep this until
          I am sure this simple approach work and no distinction
          between datatypes is needed.
'''
...
def Izeros1di(pylib,dims) :
  ...
  # Store the numpy array in a dictionary
  # with the numpy id as a key
  config.nparr[id(out)]=out
...
def Fzeros1df(pylib,dims) :
  ...
  # Cast to numpy float pointer type
...
def num1di(arr):
  ...
  # Find the id of the correponding numpy array.
  for key, value in config.descr.items():
      if value == arr:
        id = key
        break  # Exit the loop once a match is found

  # Get the array itself  
  return(config.nparr[id])
...
def num2di(arr):
  ...
  '''num2di converts a 2d int epsarray to a numpy array
  
      Parameters:
        arr     : 2D eps int array
  '''

  # Get the id for the corresponding numpy array
  for key, value in config.descr.items():
    if value == arr:
      id = key
      break  # Exit the loop once a match is found
...
def num1df(arr):
  ...
  # Find the array id from the eps pointer
  for key, value in config.descr.items():
      if value == arr:
        id = key
        break  # Exit the loop once a match is found
...
def num2df(arr):
  ...
  # Find the id for the corresponding numpy array
  for key, value in config.descr.items():
    if value == arr:
      id = key
      break  # Exit the loop once a match is found
