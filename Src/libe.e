...
# The include file libe.i contains the definition
# of the public interface of the library, while the include
# file m.i contain machine specific constants.
# The runtime library run.e declares it's public interface
#  in the  run.i include file.
...
# and strings see the libe.i include file.
...
:
  ...
  # The RunExit routine is defined in the run.i
  # include file.
...
int   LibeFillbuff(int fp):end 
   # Fill buffer                
int   LibeFlushbuff(int fp):end 
  # Flush buffer               
#
# The argument to LibeFillbuff and LibeFlushbuff
...
:
  # The LibePuti routine outputs an integer to a file
  # via the fp EFILE reference. 
...
:
  # LibePutf outputs a float to a file
  # via the fp EFILE reference. 
...
:
  # LibeWrite is the routine corresponding to
  # LibeRead for unbuffered write.
...
:
  ...
  # LibeFlushbuff writes a chunk of data to a file. 
  # If a buffer has not beeen allocated, the routine
  # attempts to allocate a buffer.
...
: 
  ...
  # This function copies the  s string into the
  # t string.
...
: 
  ...
  # This function appends the  s string to the
  #  t string.
...
: 
  # This function copies the  s string into a safe place
...
int LibeFtoaf(int mant, int nexp, int nfield, int nfrac, char [*] s):end 
 
int LibeFtoae(int mant, int nexp, int nfield, int nfrac, char [*] s):end 
 
 
int LibeAtoi(char [*] s)
:   

  # LibeAtoi returns an integer corresponding to the ascii 
  # character string  s. 
...
:   
  ...
  # Ftoa routine converts a floating point number into a
  # string.
...
:   
 ...
 # LibeFtoae  converts float to ascii with exponential notation 
...
:   
  ...
  # LibeFtoaf converts gloat to ascii with fixed point 
...
:   
  ...
  # constants defined in the  m.i include file. 
...
:   
  # LibeFabs -- Get absolute value of float 
  if(x < 0.0) 
    return(-x);
  else
    return(x);
...
:   
  ...
  # LibeFscale computes the value of $x 10^n$. 
...
float LibeSincos(float x, float y, float sign):end 
 
float LibeAtan(float f):end 
...
:
  ...
  # LibeSincos -- Sin and cosine function. 
...
: 
  ...
  # LibeMod computes the r-modulus of the
  # integer n.
