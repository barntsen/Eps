...
def int SemImport(struct tree p, struct symbol etp) :
...
def int SemDeclarations(struct tree p, struct symbol tp) :
...
def int SemStructdecl(struct tree p, struct symbol tp) :
...
def int SemArgtype(struct tree p, struct symbol tp) :
...
def int SemArray(struct tree p, struct symbol tp) :
...
def int SemStructure(struct tree p, struct symbol tp) :
  ...
  elif(LibeStrcmp(SymGetstruct(tp),"struct")):
    PtreeSetref(np,"sref") 
...
def int SemId(struct tree p) :
  ...
  elif(LibeStrcmp(SymGetype(tp),"rconstant")==OK):
    PtreeSetname(p,SymGetype(tp))
    PtreeSetdef(p,SymGetdescr(tp))
    PtreeSetype(p,"float")
    return(OK)
 
  elif(LibeStrcmp(SymGetype(tp),"sconstant")==OK):
    PtreeSetname(p,SymGetype(tp))
    PtreeSetdef(p,SymGetdescr(tp))
    PtreeSetype(p,"char")
  ...
  if(np != NULL):
    if(LibeStrcmp(PtreeGetname(np), "exprlist")):
      ...
      if(PtreeMvsister(np) != NULL):
        ...
        if(LibeStrcmp(PtreeGetstruct(p),"struct")):
          PtreeSetref(p,"sref") 
    elif(LibeStrcmp(PtreeGetname(np), "selector")):
      SemStructure(p,tp) 
 
  else:
    if(LibeStrcmp(PtreeGetarray(p),"array")):
      PtreeSetref(p,"aref") 
    elif(LibeStrcmp(PtreeGetstruct(p),"struct")):
      PtreeSetref(p,"sref") 
 
  return(OK) 


def int SemFcall(struct tree p) :
...
def int SemCopytype(struct tree p, struct tree np) :
...
def int SemCast(struct tree p) :
  ...
  if(LibeStrcmp(PtreeGetname(p),"cast")):
    ...
    elif(LibeStrcmp(PtreeGetstruct(np),"struct")):
      PtreeSetref(np,"sref") 
    ...
    if((LibeStrcmp(resultref,"aref") == ERR)|| \
      ...
      elif(LibeStrcmp(resultype,"char")):
        if(LibeStrcmp(exptype, "int") == ERR):
          SemSerror(p,"Illegal conversion", " ") 
        return(ERR) 
 
      elif(LibeStrcmp(resultype,"float")):
        if(LibeStrcmp(exptype, "int") == ERR):
...
def int SemDelete(struct tree p) :
...
def int SemComparetype(struct tree p, struct tree np) :
  ...
  elif(LibeStrcmp(PtreeGetref(p),"aref") == OK):
...
def struct tree SemRelexpr(struct tree p) :
  ...
  if(LibeStrcmp(PtreeGetdef(p),"==") == ERR):
    if(LibeStrcmp(PtreeGetdef(p),"!=") == ERR):
      ...
      elif((LibeStrcmp(PtreeGetref(rp),"aref"))|| \
        (LibeStrcmp(PtreeGetref(np),"sref"))):
         SemSerror(p,"Illegal operation", " ") 
    elif((LibeStrcmp(PtreeGetype(np), "complex"))): 
      SemSerror(p,"Illegal operation", " ") 
...
def struct tree SemAddexpr(struct tree p) :
  ...
  elif((LibeStrcmp(PtreeGetref(rp),"sref"))):
     SemSerror(np,"Illegal operation", " ") 
  else: 
    SemCopytype(np, p) 
    PtreeSetlval(p, "void") 
  ...
  if(LibeStrcmp(PtreeGetname(p), "binexpr")):
    ...
    if(SemComparetype(leftp, rightp) == ERR):
      ...
      if((LibeStrcmp(PtreeGetdef(np), "="))):
        SemAsgexpr(np)  
      elif((LibeStrcmp(PtreeGetdef(np), "!=")) || \
...
def struct tree SemPrimexpr(struct tree p) :
  ...
  if(LibeStrcmp(PtreeGetname(p),"identifier")):
    SemId(p) 
    return(p) 
  elif(LibeStrcmp(PtreeGetname(p), "fcall")):
    SemSetsimple(ERR)
    SemFcall(p) 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "cast")):
    SemSetsimple(ERR)
    SemCast(p) 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "new")):
    SemSetsimple(ERR)
    SemNew(p) 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "delete")):
    SemSetsimple(ERR)
    SemDelete(p) 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "cmplx")):
    SemSetsimple(ERR)
    SemCmplx(p) 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "re")):
    SemSetsimple(ERR)
    SemRe(p) 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "len")):
    SemSetsimple(ERR)
    SemLen(p) 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "im")):
    SemSetsimple(ERR)
    SemIm(p) 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "sizeof")):
    SemSetsimple(ERR)
    SemSizeof(p) 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "iconstant")):
    PtreeSetype(p, "int") 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "rconstant")):
    PtreeSetype(p, "float") 
    return (p) 
  elif(LibeStrcmp(PtreeGetname(p), "sconstant")):
    SemSetsimple(ERR)
    PtreeSetype(p, "char") 
    PtreeSetref(p, "aref") 
    PtreeSetarray(p, "array") 
...
def int Semisnobody(struct tree p) :
  ...
  if(LibeStrcmp(PtreeGetname(np),"arglist")==OK):
    ...
    if(np==NULL):
      ...
      return(ERR) 
  elif(LibeStrcmp(PtreeGetname(np),"compstmnt")==OK):
    np=PtreeMvchild(np)      #Move to the function body
    if(np==NULL):
      return(OK) 
    else:
...
def int SemExtdecl(struct tree p) :
  ...
  if(LibeStrcmp(PtreeGetname(p), "extdecl")):         
   ...
   # Check function declaration  
      elif(LibeStrcmp(PtreeGetname(sp), "fdef")):
